...
# helper function to check for collisions between rocks and another object
...
def draw(canvas):
    ...
    # call helper function to draw and update sprites
    process_sprite_group(canvas, rock_group)
    process_sprite_group(canvas, missile_group)
    process_sprite_group(canvas, explosion_group)
    
    # call helper function to check for collisions between rocks and ship
    # and update lives
    if group_collide(rock_group, my_ship):
        lives -= 1
    
    # call helper function to check for collisions between missiles and rocks
    # and update score
    score += group_group_collide(missile_group, rock_group)
    
    # end game if lives expire
    if lives == 0:
        started = False
        rock_group = set([])
        soundtrack.rewind()
...
def rock_spawner():
    ...
    rock_vel = ([(random.random() * 2 - 1) * (6 - lives), (random.random() * 2 - 1) * (6 - lives)])
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if (len(rock_group) < 12) and (dist(my_ship.pos, a_rock.pos) > (4 * my_ship.radius)) and started:
